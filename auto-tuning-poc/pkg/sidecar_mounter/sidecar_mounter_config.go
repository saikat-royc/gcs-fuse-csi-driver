...
const (
	...
	fileCacheMedium      = "file-cache-medium"
...
func NewMountConfig(sp string) *MountConfig {
	...
	klog.Infof("file cache medium received: %s", mc.FileCacheMedium)
	if mc.BucketName == "" {
...
func (mc *MountConfig) prepareMountArgs() {
	...
	klog.Infof("mc.mountoptions %v", mc.Options)
	for _, arg := range mc.Options {
		...
		if flag == fileCacheMedium {
			mc.FileCacheMedium = value
			klog.Infof("setting file cache medium in mount config:%s", value)
			continue
		}
	...
	// if cache dir is set, reset it to the right path for a chosen medium. we make that decision in L242
	if mc.FileCacheMedium == mediumRAM {
		configFileFlagMap["cache-dir"] = filepath.Join(webhook.SidecarContainerCacheRamDiskVolumeMountPath, ".volumes", mc.VolumeName)
		mc.CacheDir = filepath.Join(webhook.SidecarContainerCacheRamDiskVolumeMountPath, ".volumes", mc.VolumeName)
		klog.Infof("updating cachedir path to %s", configFileFlagMap["cache-dir"])
	}
	if mc.FileCacheMedium == mediumLSSD {
		configFileFlagMap["cache-dir"] = filepath.Join(webhook.SidecarContainerCacheEphemeralDiskMountPath, ".volumes", mc.VolumeName)
		mc.CacheDir = filepath.Join(webhook.SidecarContainerCacheEphemeralDiskMountPath, ".volumes", mc.VolumeName)
		klog.Infof("updating cachedir path to %s", configFileFlagMap["cache-dir"])
	}

	if mc.FileCacheMedium == mediumPDSSD {
		configFileFlagMap["cache-dir"] = filepath.Join(webhook.SidecarContainerCacheVolumeMountPath, "mount", ".volumes", mc.VolumeName)
		mc.CacheDir = filepath.Join(webhook.SidecarContainerCacheVolumeMountPath, "mount", ".volumes", mc.VolumeName)
		klog.Infof("updating cachedir path to %s", configFileFlagMap["cache-dir"])
	}
